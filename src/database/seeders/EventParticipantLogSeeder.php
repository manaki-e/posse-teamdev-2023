...
class EventParticipantLogSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $events = new Event();
        $maximum_event_participate_fee = Setting::maximumEventParticipatePoint();
        foreach ($event_ids as $event_id) {
            ...
            foreach ($random_user_ids as $user_id) {
                $event_participants_array[] = [
                    'event_id' => $event_id,
                    'user_id' => $user_id,
                    'point' => $faker->numberBetween(0, $maximum_event_participate_fee),
        ...
        $all_completed_events = $events->completedEvents()->with('eventParticipantLogs')->get();
        $all_completed_events->each(function ($event) {
            $point_sum = 0;
            $event->eventParticipantLogs->each(function ($participant) use (&$point_sum) {
