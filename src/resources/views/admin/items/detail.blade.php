                ...
                <div class="w-full mb-4">
                    <ul class="flex gap-2">
                        <li class="w-1/4 flex-center">
                            <div @click="activeImage = activeImage + 2"
                                class="rounded-full overflow-hidden bg-gray-200 p-3 cursor-pointer hover:scale-125">
                        ...
                        <li class="">
                            ...
                            <div :class="{ '!block': activeImage % {{ count($product -> productImages) }} === {{ $loop->index }} }"
                                x-show.transition.in.opacity.duration.600="activeImage % {{ count($product -> productImages) }} === {{ $loop->index }}"
                                class="hidden">
                    ...
                    <ul class="flex flex-col gap-2 p-0">
                        ...
                        <li class="flex items-center gap-4 pl-4">
                            <p
                                ...
                                利用ポイント:</p>
                            <p class="block antialiased font-sans text-sm leading-normal font-normal text-gray-500">
                                {{ empty( $product -> point ) ? '未設定' : $product->point .' pt'}}
                            </p>
                        </li>
                        <li class="flex items-center gap-4 pl-4">
                            <p
                        ...
                        <li class="flex items-center gap-4 pl-4">
                            ...
                            <p class="block antialiased font-sans text-sm leading-normal font-normal text-gray-500">
                                ...
                                {!! ( $product -> request )
                                ? 'あり <a href="#"
                                    class="hover:text-blue-700 border-b border-blue-800">（ここを押すと紐づけられたリクエストの詳細に飛びます）</a>'
                                : 'なし' !!}
                            </p>
                        </li>
                        <li class="flex items-center gap-4 pl-4">
                        ...
                        <li class="flex items-center gap-4 pl-4">
                            <p
                                ...
                                貸出者:</p>
                            <p class="block antialiased font-sans text-sm leading-normal font-normal text-gray-500">
                                <a href="{{ route('admin.users.show', ['user' => $product -> user -> id]) }}"
                                    class="hover:text-blue-700 border-b border-blue-600">{{ $product -> user -> name }}</a>
                            </p>
                        </li>
                        <li class="flex items-center gap-4 pl-4 mt-4">
                            @if ($product -> status === 1)
                            <x-admin-button-edit
                                action="{{ route('admin.items.update', ['item' =>  $product -> id]) }}">
                                <x-slot name="content">
                                    ポイントを設定して承認する
                                </x-slot>
                                ...
                                <x-slot name="form_slot">
                                    <div class="mb-4">
                                        <div class="relative flex gap-4">
                                            <label for="point"
                                                class="leading-7 text-sm text-gray-600 flex-center">Point:</label>
                                            <input type="text" id="point" name="point" value="{{ $product -> point }}"
                                                class="bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                        </div>
                                    </div>
                                </x-slot>
                            ...
                            <x-admin-button-delete
                                action="{{ route('admin.items.destroy', ['item' =>  $product -> id]) }}">
                                <x-slot name="modal_title">
                        ...
                        <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
                            ...
                            <tbody class="divide-y divide-gray-100 border-t border-gray-100">
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4">
                                        <a href="{{ route('admin.users.show', ['user' => $deal -> user -> id]) }}"
                                            class="hover:text-blue-700 border-b border-blue-600">{{ $deal -> user -> name }}</a>
                                    </td>
